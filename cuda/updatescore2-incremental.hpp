   ...
   noexcept
{
   ...
   int res = el(subst,SUBSTSZ, seqY[i], seqX[j]) + insdelcost;

   res =       res + el(score,cols, i-1,j-1)  ;   // UP LEFT
   res = max2( res,  el(score,cols, i-1,j  ) );   // LEFT
   res = max2( res,  el(score,cols, i  ,j-1) );   // UP
   
   res -= insdelcost;

   el(score,cols, i,j) = res;
}
