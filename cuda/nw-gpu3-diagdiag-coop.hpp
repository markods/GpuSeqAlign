...
void Nw_Gpu3_DiagDiag_Coop( NWArgs& nw, NWResult& res )
{
   ...
   // launch kernel A
   {
      ...
      dim3 gridA;
      gridA.y = ceil( float( nw.adjrows )/nw_gpu3::tileAy );
      gridA.x = ceil( float( nw.adjcols )/nw_gpu3::tileAx );
      // block dimensions for kernel A
      dim3 blockA { nw_gpu3::tileAx, nw_gpu3::tileAy };
      ...
      nw_gpu3::kernelA<<< gridA, blockA, 0, 0/*STREAM_ID*/ >>>( seqX_gpu, seqY_gpu, score_gpu, nw.adjrows, nw.adjcols, ( int (*)[SUBSTSZ] )subst_gpu, nw.insdelcost );
   ...
   // launch kernel B
   {
      ...
      int trows = ceil( float( nw.adjrows-1 )/nw_gpu3::tileBy );
      int tcols = ceil( float( nw.adjcols-1 )/nw_gpu3::tileBx );
      ...
      // calculate grid and block dimensions for kernel B
      {
         ...
         blockB.x  = ceil( min( nw_gpu3::tileBy, nw_gpu3::tileBx )*1./WARPSZ )*WARPSZ;
         blockB.x *= ceil( max( nw_gpu3::tileBy, nw_gpu3::tileBx )*2./WARPSZ );
         ...
         int sharedMemSz = ( ( 1+nw_gpu3::tileBy )*( 1+nw_gpu3::tileBx ) )*sizeof( int );

         // calculate the max number of parallel blocks per streaming multiprocessor
         cudaOccupancyMaxActiveBlocksPerMultiprocessor( &maxBlocksPerSm, nw_gpu3::kernelB, numThreads, sharedMemSz );
      ...
      // +   update the stop event when the kernel finishes
      cudaEventRecord( start, 0/*STREAM_ID*/ );
      cudaLaunchCooperativeKernel( ( void* )nw_gpu3::kernelB, gridB, blockB, kargs, 0, 0/*STREAM_ID*/ );
      cudaEventRecord( stop, 0/*STREAM_ID*/ );
      cudaEventSynchronize( stop );
