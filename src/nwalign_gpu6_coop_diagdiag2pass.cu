...
NwStat NwAlign_Gpu6_Coop_DiagDiag2Pass(const NwAlgParams& pr, NwAlgInput& nw, NwAlgResult& res)
{
    ...
    // launch kernel A
    {
        ...
        if (cudaSuccess != (res.cudaStat = cudaLaunchKernel((void*)Nw_Gpu6_KernelA, gridA, blockA, kargs, shmemsz, cudaStreamDefault)))
        {
    ...
    // launch kernel B
    {
        ...
        // launch the kernel in the given stream (don't statically allocate shared memory)
        if (cudaSuccess != (res.cudaStat = cudaLaunchCooperativeKernel((void*)Nw_Gpu6_KernelB, gridB, blockB, kargs, shmemsz, cudaStreamDefault)))
        {
            return NwStat::errorKernelFailure;
        }
    }
